...
plot_behaviours <- function(behaviourList, formatted_data, Experiment_path, n_samples, n_col) {
  behaviourList <- selectedBehaviours
...
## checking frequency of behaviours
behaviourfrequency <- formatted_data %>%
  group_by(activity) %>%
  summarise(frequency = n()) %>%
  arrange(desc(frequency))

ggplot(behaviourfrequency, aes(x = reorder(activity, -frequency), y = frequency)) +
  geom_bar(stat = "identity") +
  labs(title = "Frequency of Behaviours", x = "Behaviour", y = "Frequency") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

## checking frequency in behaviour classes
behaviourgroupfreq <- list(
  Inactive = c("Tree Sitting", "Sleeping/Resting", "Ground Sitting"),
  Tree_Movement = c("Tree Movement", "Branch Walking", "Swinging/Hanging", "Grooming", "Climbing Down", "Climbing Up", "Rapid Climbing"),
  Eating = c("Foraging/Eating"),
  Walking = c("Walking", "Trot", "Gallop", "Bound/Half-Bound")
)

formatteddatafreqtest <- formatted_data
formatteddatafreqtest <- formatteddatafreqtest %>%
  mutate(behaviour_group = case_when(
    activity %in% behaviourgroupfreq$Inactive ~ "Inactive",
    activity %in% behaviourgroupfreq$Tree_Movement ~ "Tree Movement",
    activity %in% behaviourgroupfreq$Eating ~ "Eating",
    activity %in% behaviourgroupfreq$Walking ~ "Walking",
    TRUE ~ "Other"
  ))

# Summarize the frequency of each behaviour group
behaviour_summary <- formatteddatafreqtest %>%
  group_by(behaviour_group) %>%
  summarize(frequency = n())

# Plot the frequency of each behaviour group
ggplot(behaviour_summary, aes(x = behaviour_group, y = frequency, fill = behaviour_group)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  labs(title = "Frequency of Behaviours by Group", x = "Behaviour Group", y = "Frequency")

rm(behaviour_summary)
rm(behaviourfrequency)
rm(formatteddatafreqtest)
rm(behaviourgroupfreq)
