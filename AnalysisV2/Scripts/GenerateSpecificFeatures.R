...
generateSpecificTsFeatures <- function(data, specific_features) {
  ...
  feature_mapping_dt <- data.table(
    output_feature = c("x_acf10", "firstmin_ac", "crossing_points", "firstzero_ac", 
    ...
    tsfeatures_name = c("acf_features", "acf_features", "crossing_points", "acf_features", 
                        "flat_spots", "max_level_shift", "max_var_shift", 
                        "max_kl_shift", "max_kl_shift", "nonlinearity", 
                        "scal_features", "scal_features", "scal_features")
  )
...
generateSpecificStatisticalFeatures <- function(window_chunk, down_Hz, specific_features) {
  ...
  # Compute ODBA if required & VDBA always (as we use it as a proxy of energy)
    ...
    if ("minODBA" %in% parsed_features) {
      set(result, j = "minODBA", value = min(ODBA, na.rm = TRUE))
    }
    if ("maxODBA" %in% parsed_features) {
