...
setwd("C:/Users/uqgspar1/OneDrive - The University of Queensland/Documents/MATLAB/Koala_Accelerometry/Scripts/Gabbys Versions/Model Running 2024") # scripts location
...
# read in data and format to standardised shape
  ...
  # balance and select our data
  subsetted_data <- MoveData0[MoveData0$ID %in% c("Elsa", "Meeka", "Hardy", "Nicole"), ]
...
behaviours <- MovementData[["behaviours_5"]]
relabelled_data <- relabel_activities(tstDat, behaviours)
relabelled_data <- relabelled_data[relabelled_data$activity != "NA", ]
processed_data <- process_data(relabelled_data, featuresList, window_length = 2, overlap_percent = 50, 20) # last one is down_Hz
tstDat2 <- processed_data %>% select(-ID)

optimal_results <- verify_optimal_results(tstDat2, OptimalMLModel, test_type = "random", # random
